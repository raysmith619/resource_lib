...
class TkRemHost:
    ...
    def cmd_in_th_proc(self):
        while True:
            ...
            # Waiting till user establishes connection
            if self.sock_out is None:            
                self.sock_out = sk.socket(sk.AF_INET, sk.SOCK_STREAM)
                SlTrace.lg(f"HOST: self.sock_out.connect(({self.host}, {self.port_out}))", "HOST")
                self.sock_out.connect((self.host, self.port_out))
            
            #self.sock_out = self.connection
    ...
    def cmd_proc(self, cmd_dt):
        ...
        ret_dt['ret_val'] = ret
        SlTrace.lg(f"HOST: ret_dt:{ret_dt}", "HOST")
        ret_data = pickle.dumps(ret_dt)
        #self.sock_out.connect((self.host, self.port_out))
        self.sock_out.send(ret_data)
