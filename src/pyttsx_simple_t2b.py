#pyttsx_simple_t2b.py   07Aug2023  crs
""" Simple talk
    engine talking inside thread
"""
import time
import threading
import queue
        ...
        while self.running:
            ...
            if (not self.is_talking
                ...
                print(f"pyt_thread text: {text}"
                    f" pyt_queue_qsize(): {self.pyt_queue.qsize()}"
                    f" is_talking: {self.is_talking}")
                self.pytr_thread = threading.Thread(target=self.pytr_thread_proc,
                                                    args=[text])
                self.pytr_thread.run()

    def pytr_thread_proc(self, text):
