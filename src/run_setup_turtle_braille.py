#run_setup_turtle_braille.py    26Jul2024  crs, Author
...
print(f"setup_pgm: {setup_pgm}")
if not os.path.exists(setup_pgm):
    print(f"setup program not found")
    exit(1)

subprocess.run(['python', setup_pgm])                # Just trial
#subprocess.run(['python', setup_pgm, "--verbose"])  # lots of info
#subprocess.run(['python', setup_pgm, "--update"])  # Update
