# wx_tb_test_run.py    11jul2024 crs, run wx_tb_test_list
"""
...
num_not_found = 0       # Count missing test files
...
with open(out_file, "w") as outfp:
    ...
    for line in tlf_lines:
        ...
        if not os.path.exists(pgm_file):
            num_not_found += 1
            print(f"test file not found: {num_not_found}: {pgm_file}")
            continue
        ...
        try:
            ...
            SlTrace.lg(f"Uncaught exception: {e}", to_stdout=True)
    ...
    SlTrace.lg(f"\nEnd of Testing Run time: {run_time_end-run_time_start:.1f}",
                to_stdout=True)
    if num_not_found > 0:
        SlTrace.lg(f"{num_not_found} missing test files", to_stdout=True)
